...
-- DEFAULT 값 : 특정 속성의 기본 값 설정
-- UNIQUE : 중복 허용하지 않음, NULL 허용되며 하나의 테이블당 여러개 가능 (PK와 차이점)
...
-- drop : 테이블 존재 자체를 없앰
DROP TABLE product;
-- truncate : 테이블의 구조만 남기고 모든 행 삭제 (데이터만 삭제)
TRUNCATE TABLE product;

-- 5. 테이블 정의 수정 : 이미 존재하는 테이블에 데이터가 추가되었을 때 or 컬럼 정보가 바뀌어야 하는 경우에 사용 
-- 5-1. 컬럼 추가
ALTER TABLE product ADD new_column DATE;

-- 5-2. 컬럼 수정
ALTER TABLE product MODIFY new_column INT;
ALTER TABLE product CHANGE new_column new_column2 INT; -- 컬럼명을 변경시에는 change 원래명 수정명;
